...
{
  //Genero il codice per la condizione, salvo il blocco su cui sto scrivendo per il PhiNode. 
  BasicBlock *entryBB = builder->GetInsertBlock();
  Value *CondV = CondExpr->codegen(drv);
  if (!CondV)
    return nullptr;

  //Creo il TrueBB, il FalseBB viene creato solo se è presente l'else.
  //Inserisco solo il TrueBB. 
  Function *function = builder->GetInsertBlock()->getParent();
  ...
  //Il MergeBB invece è il blocco di "fine" che unirà i due flussi. 
  BasicBlock *MergeBB = BasicBlock::Create(*context, "endstmt");

  //Se è presente l'else -> se condV è vera salto a TrueBB altrimenti a FalseBB
  //Se NON è presente l'else -> se condV è vera salto a TrueBB altrimenti a MergeBB
  if (ElseStmt)
    builder->CreateCondBr(CondV, TrueBB, FalseBB);
  else
    builder->CreateCondBr(CondV, TrueBB, MergeBB);

  //Dopo aver settato l'insert point in TrueBB genero il codice. 
  builder->SetInsertPoint(TrueBB);
  Value *TrueV = TrueStmt->codegen(drv);
  if (!TrueV)
    return nullptr;

  //Salto incodizionato verso il blocco "fine" alla fine del TrueBB
  builder->CreateBr(MergeBB);

  //Inserisco il FalseBB o il MergeBB in base a che ci sia l'Else o meno. 
  TrueBB = builder->GetInsertBlock();
  if (ElseStmt)
    function->insert(function->end(), FalseBB);
  else
  ...
  //Se è presente l'else genero il rispettivo codice dopo aver settato l'insert point. 
  Value *FalseV;
  if (ElseStmt)
  {
    builder->SetInsertPoint(FalseBB);
  ...
  //Altrimenti dopo aver settato l'insert point in MergeBB
  builder->SetInsertPoint(MergeBB);

  //Creo il phi node che unirà sempre due flussi. 
  // - Se è presente l'else: 
  //    - Se arrivo dal TrueBB allora il valore di ritorno sarà TrueV
  //    - Se arrivo dal FalseBB allora il valore di ritorno sarà FalseV
  // - Se NON è presente l'else: 
  //    - Se arrivo dal TrueBB allora il valore di ritorno sarà TrueV
  //    - Se arrivo dal EntryBB (condizione falsa) allora il valore di ritorno sarà 0.0
  PHINode *PN = builder->CreatePHI(Type::getDoubleTy(*context), 2, "condval");
  PN->addIncoming(TrueV, TrueBB);
  if (ElseStmt)
    PN->addIncoming(FalseV, FalseBB);
...
InitFor::InitFor(initType InitExp) : InitExp(InitExp) {};

initType InitFor::getOp()
{
  return InitExp;
}
...
ForStmtAST::ForStmtAST(InitFor* Init, ExprAST* CondExpr, AssignmentAST* AssignExpr, StmtAST* BodyStmt) : Init(Init), CondExpr(CondExpr), AssignExpr(AssignExpr), BodyStmt(BodyStmt){};

Value *ForStmtAST::codegen(driver &drv){
  //Generazione codice condizione per init.
  //Nell'init ci potranno essere due casi: 
  //-> caso assignement -> InitExp è un assignment
  //-> caso VarBinding -> InitExp è un varBinding
  //tmpAlloca è una variabile che servirà per ripristinare lo scope esterno una volta terminato il ciclo. 
  AllocaInst *tmpAlloca;
  if(Init->getOp().index()){//Caso Assignment, la codegen ritorna un Value
    Value *InitV = std::get<AssignmentAST*>(Init->getOp())->codegen(drv);
    if (!InitV)
      return nullptr;
  }
  else{ //Caso VarBinding, la codegen ritorna un'AllocaInst* che, a differenza del Value, andrà "gestita".
    AllocaInst *boundval = std::get<VarBindingAST*>(Init->getOp())->codegen(drv);
    if (!boundval)
      return nullptr;
    tmpAlloca = drv.NamedValues[std::get<VarBindingAST*>(Init->getOp())->getName()];
    drv.NamedValues[std::get<VarBindingAST*>(Init->getOp())->getName()] = boundval;
  }
  ...
  //Se condV è vera jump al LoopBB
  //Se condV è falsa jump al MergeBB
  ...
  //Genero il codice per l'Assignment
  ...
  if(!Init->getOp().index())
    drv.NamedValues[std::get<VarBindingAST*>(Init->getOp())->getName()] = tmpAlloca;
...
Value* LogicalExprAST::codegen(driver &drv) {
    ...
    // Determino il valore dell'operazione
    if (Op == "and" || Op == "or") {
        builder->SetInsertPoint(FalseBB);
        builder->CreateBr(MergeBB);
...
// /**************ArrayBindingAST*************************/
// ArrayBindingAST::ArrayBindingAST(const std::string&Name, ExprAST* Size)
//     : VarBindingAST(Name, nullptr), Size(Size){}

// ArrayBindingAST::ArrayBindingAST(const std::string& Name, ExprAST* Size, std::vector<ExprAST*> InitValues = {})
...
//     Value *SizeVal = Size->codegen(drv);
//     if (!SizeVal)
//         return nullptr;
//       // Convert size to a 64-bit integer (assuming 64-bit architecture)
//       SizeVal = builder->CreateFPToUI(SizeVal, Type::getInt64Ty(*context), "sizevaltmp");
//     if (auto *CI = dyn_cast<ConstantInt>(SizeVal)) {
//       uint64_t ArraySize = CI->getZExtValue();
//       // Create an array type of 'double' with the specified size
//       ArrayType *arr_type = llvm::ArrayType::get(Type::getDoubleTy(*context), ArraySize);

//       // Allocate memory for the array in the stack
//       AllocaInst *Alloca = CreateEntryBlockAlloca(fun, this->getName(), arr_type );

//       // If initial values are provided, generate code to initialize the array
//       if (!InitValues.empty()) {
//             for (unsigned i = 0, e = InitValues.size(); i != e; ++i) {
//                 // Generate code for each initial value
//                 Value *InitVal = InitValues[i]->codegen(drv);
//                 if (!InitVal)
//                     return nullptr;

//                 // Create an index for accessing the array element
//                 Value *Index = ConstantInt::get(Type::getInt64Ty(*context), i);
//                 Value *ElemPtr = builder->CreateInBoundsGEP(arr_type, Alloca, {Index}, "arrayidx");

//                 // Store the initial value in the array element
...
// /**************ArrayAccessExprAST*****************/
// ArrayAccessExprAST::ArrayAccessExprAST(const std::string& ArrayName, ExprAST* Index)
//         : ArrayName(ArrayName), Index(Index) {}
// ArrayAccessExprAST::ArrayAccessExprAST(const std::string& ArrayName, ExprAST* Index, ExprAST* Val)
//         : ArrayName(ArrayName), Index(Index), Val(Val) {}

// Value *ArrayAccessExprAST::codegen(driver& drv){
//         // Retrieve the array allocation instruction from NamedValues
//         AllocaInst *ArrayAlloca = dyn_cast<AllocaInst>(drv.NamedValues[ArrayName]);
//         if (!ArrayAlloca) {
//             // Handle error: Array allocation not found
//             return nullptr;
//         }

//         // Generate the code for the index expression
//         auto *IndexVal = Index->codegen(drv);
//         if (!IndexVal)
//             return nullptr;

//         // Convert index to an integer (if necessary)
//         if (IndexVal->getType()->isDoubleTy()) {
//             IndexVal = builder->CreateFPToUI(IndexVal, Type::getInt32Ty(*context), "idxtemp");
//         }

//         ArrayType *arrType = dyn_cast<ArrayType>(ArrayAlloca->getAllocatedType());
//         if (!arrType) {
...
//         // Create a GEP (Get Element Pointer) instruction to compute the address of the array element
//         std::vector<Value *> Indices;
//         Indices.push_back(ConstantInt::get(Type::getInt32Ty(*context), 0)); // array is a pointer; this is the first dereference
//         Indices.push_back(IndexVal); // this is the index into the array
//         Value *ElemPtr = builder->CreateInBoundsGEP(elementType, ArrayAlloca, Indices, "elementPtr");

//         // Load the value at the computed address
//         return builder->CreateLoad(elementType, ElemPtr, "loadElement");
// }


// /**************ArrayAssigmentAST*/
// ArrayAssignmentAST::ArrayAssignmentAST(const std::string &ArrayName, ExprAST* Index, ExprAST* Val)
//         : AssignmentAST(ArrayName, Val), Index(Index), Val(Val) {}

// Value *ArrayAssignmentAST::codegen(driver& drv) {
  //  AllocaInst *ArrayAlloca = dyn_cast<AllocaInst>(drv.NamedValues[ArrayName]);
  //   if (!ArrayAlloca) {
  //       // Handle error: Array not found
  //       return LogErrorV("Array " + ArrayName + " not found");
  //   }
  ...
  //       Value *Val = this->Val->codegen(drv);
  //   if (!Val) {
  //       return nullptr;
//     }
...
//     std::vector<Value *> Indices;
//     Indices.push_back(ConstantInt::get(Type::getInt32Ty(*context), 0)); // array is a pointer; first index is 0
//     Indices.push_back(IndexVal); // index into the array
//     Value *ElemPtr = builder->CreateInBoundsGEP(elementType, ArrayAlloca, Indices, "elementPtr");

//     // Store the value at the computed address
//     builder->CreateStore(Val, ElemPtr);

//     // Return the assigned value (or nullptr if your language semantics dictate so)
//     return Val;
// }

ArrayAssignmentAST::ArrayAssignmentAST(std::string VarName, ExprAST *Index, ExprAST *Val) 
    : VarName(VarName), Index(Index), Val(Val){};

Value *ArrayAssignmentAST::codegen(driver &drv)
{
    auto my_arr = drv.NamedValues[VarName];
    if (!my_arr)
    {
    ...
    Value *indexInteger = builder->CreateFPToUI(idx, Type::getInt32Ty(*context), "indexInteger");

    Value *offsetPosition = builder->CreateInBoundsGEP(Type::getDoubleTy(*context), my_arr, indexInteger, "arrayAccess");

    auto val = Val->codegen(drv);
    if (!val)
        return nullptr;

    return builder->CreateStore(val, offsetPosition);
}

ArrayAccessExprAST::ArrayAccessExprAST(std::string VarName,ExprAST *Index) 
...
Value *ArrayAccessExprAST::codegen(driver &drv)
{
    ...
    Value* offsetPosition = builder->CreateInBoundsGEP(Type::getDoubleTy(*context),my_arr,indexInteger,"arrayAccess");
    return builder->CreateLoad(Type::getDoubleTy(*context), offsetPosition, "accessedValue");
}


ArrayDeclarationAST::ArrayDeclarationAST(const std::string &varName, NumberExprAST* size)
    : VarName(varName), Size(size) {}

ArrayDeclarationAST::ArrayDeclarationAST(const std::string &varName, NumberExprAST* size, std::vector<ExprAST*> values)
    : VarName(varName), Size(size), Values(std::move(values)) {}

Value *ArrayDeclarationAST::codegen(driver &drv) {
    ...
    // if (!sizeVal) 
    //   return nullptr;
    if (!llvm::isa<llvm::ConstantInt>(Size->codegen(drv))) {
          LogErrorV("Array size must be a constant");
          return nullptr;
      }

    
    uint64_t arraySize = static_cast<uint64_t>(llvm::cast<llvm::ConstantInt>(Size->codegen(drv))->getZExtValue());

    uint64_t dim = static_cast<uint64_t>(llvm::cast<llvm::ConstantInt>(Size->codegen(drv))->getZExtValue());


    ArrayType *arrayType = ArrayType::get(Type::getDoubleTy(*context), arraySize);

    AllocaInst *allocatmp = CreateEntryBlockAlloca(fun, VarName, arrayType);


    if (!Values.empty()) {
        for (size_t i = 0; i < Values.size(); ++i) {
            // Generate code for each value in the initializer
            Value *initVal = Values[i]->codegen(drv);
            if (!initVal) return nullptr;

            // Create index for accessing the array element
            Value *index = ConstantInt::get(*context, APInt(32, static_cast<uint64_t>(i), true));

            // Calculate pointer to the i-th element
            Value *elemPtr = builder->CreateInBoundsGEP(arrayType, allocatmp, ArrayRef<Value*>({index}), "elemPtr");

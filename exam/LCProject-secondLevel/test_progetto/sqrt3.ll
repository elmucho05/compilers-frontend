...
condstmt:                                         ; preds = %entry
  ...
  br i1 %lttest, label %then, label %else
...
ifcont:                                           ; preds = %loopstmt, %else, %then
  %iftmp = phi i1 [ %lttest12, %then ], [ false, %else ]
  %nottmp = xor i1 %iftmp, true
  br i1 %nottmp, label %loopstmt, label %mergestmt

loopstmt:                                         ; preds = %ifcont

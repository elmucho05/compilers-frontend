...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // globalvar
      char dummy6[sizeof (GlobalVarAST*)];

      // ifstmt
      char dummy7[sizeof (IfStmtAST*)];

      // init
      char dummy8[sizeof (InitFor*)];
      ...
      // binding
      char dummy12[sizeof (VarBindingAST*)];
      ...
      // optexp
      // explist
      char dummy15[sizeof (std::vector<ExprAST*>)];

      // stmts
      char dummy16[sizeof (std::vector<StmtAST*>)];

      // vardefs
      char dummy17[sizeof (std::vector<VarBindingAST*>)];
    ...
    TOK_LBRACE = 274,              // "{"
    TOK_RBRACE = 275,              // "}"
    TOK_EXTERN = 276,              // "extern"
    TOK_DEF = 277,                 // "def"
    TOK_VAR = 278,                 // "var"
    TOK_GLOBAL = 279,              // "global"
    TOK_IF = 280,                  // "if"
    TOK_ELSE = 281,                // "else"
    TOK_FOR = 282,                 // "for"
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_block = 42,                            // block
        S_vardefs = 43,                          // vardefs
        S_binding = 44,                          // binding
        S_exp = 45,                              // exp
        S_initexp = 46,                          // initexp
        S_expif = 47,                            // expif
        S_condexp = 48,                          // condexp
        S_idexp = 49,                            // idexp
        S_optexp = 50,                           // optexp
        S_explist = 51,                          // explist
        S_ifstmt = 52,                           // ifstmt
        S_init = 53,                             // init
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_init: // init
        value.move< InitFor* > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, InitFor*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const InitFor*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_init: // init
        value.template destroy< InitFor* > ();
        break;
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_init: // init
        value.copy< InitFor* > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_init: // init
        value.move< InitFor* > (YY_MOVE (s.value));
        break;

...
static AllocaInst *CreateEntryBlockAlloca(Function *fun, StringRef VarName) {
  IRBuilder<> TmpB(&fun->getEntryBlock(), fun->getEntryBlock().begin()); //dice che nel basic block, le istruzioni saranno salvate all'inizio 
...
Value *BinaryExprAST::codegen(driver& drv) {
  ...
  case '/':
    return builder->CreateFDiv(L,R,"divideres");
  case '<':
...
Value* IfExprAST::codegen(driver& drv) {
    ...
    // memorizza il risultato (di tipo 0 oppure 1, dunque booleano) nel registro SSA 
    ...
    if (!TrueV)
       return nullptr;
    builder->CreateBr(MergeBB); //jumps from the end of the TrueBlock to the start of Merge block
    ...
    TrueBB = builder->GetInsertBlock(); //inserisco nel valore del Blocco True da quale blocco sono arrivato
    function->insert(function->end(), FalseBB);//FalseBB va per forza inserito nella funzione, facendo così sai dove va a finire
    
    // "Posizioniamo" il builder all'inizio del blocco false, 
    // generiamo ricorsivamente il codice da eseguire in caso di
    // condizione falsa e, in chiusura di blocco, generiamo il salto 
    // incondizionato al blocco merge
...
Value* BlockExprAST::codegen(driver& drv) {
   ...
   std::vector<AllocaInst*> AllocaTmp; //Vettore dove salvo la Symbol Table
   for (int i=0, e=Def.size(); i<e; i++) {
      ...
      AllocaInst *boundval = Def[i]->codegen(drv); //valuta le espressioni trovate per ogni entry della symbol table
      //a = a+1 --> devo valutare l'espressione, prima la parte destra sicuramente. E' di tipo alloca inst perché la symbol table ha istruzioni alloca  
      if (!boundval) 
         return nullptr;
      ...
      AllocaTmp.push_back(drv.NamedValues[Def[i]->getName()]); //salvo il valore trovato nella symbol table dentro a tmp
      drv.NamedValues[Def[i]->getName()] = boundval; //poi cambio il valore nella Symbol table aggiornandolo al valore trovato
   };
   ...
   Value *blockvalue = Val->codegen(drv); //questo sarà il valore finale da ritornare per il blocco
      if (!blockvalue)
         return nullptr;
   // Prima di uscire dal blocco, si ripristina lo scope esterno al costrutto
   for (int i=0, e=Def.size(); i<e; i++) {
        drv.NamedValues[Def[i]->getName()] = AllocaTmp[i]; //ripristino della symbol table copiando quello che avevo salvato in tmp
   };
...
Function *FunctionAST::codegen(driver& drv) {
  ...
  Function *function = 
      module->getFunction(std::get<std::string>(Proto->getLexVal())); //prendo il nome della funzione come stringa
